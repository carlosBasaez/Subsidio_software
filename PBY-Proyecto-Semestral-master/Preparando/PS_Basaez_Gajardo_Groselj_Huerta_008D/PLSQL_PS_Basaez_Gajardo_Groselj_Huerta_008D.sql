...
/
-- MANDAR ERROR
CREATE OR REPLACE PROCEDURE prc_PostulantePuntaje_Error 
(p_sub_programa VARCHAR2, p_codigo VARCHAR2, p_mensaje VARCHAR2)
IS
  v_sub_programa VARCHAR2(100) := '';
  v_mensaje VARCHAR2(500) := '';
BEGIN

  v_sub_programa := SUBSTR(p_sub_programa, 1, 100);
  v_mensaje := SUBSTR('ErrorCodigo: ' || p_codigo || ' >> Mensaje: ' || p_mensaje  ,1,500);
  INSERT INTO ERRORES_PROC_ASIG_PUNTAJE 
  VALUES (
    SEQ_ERRORES_PROC_ASIG_PUNTAJE.NEXTVAL,
    v_sub_programa,
    v_mensaje);
EXCEPTION
  WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('ERROR EN LA INSERCION DE ERRORES DE ASIG PUNTAJE');
END;
...
BEGIN
  ...
  FOR REG_POSTULANTE IN (SELECT * FROM POSTULANTE) LOOP
    --BLOQUE ANIDADO PARA CONTROLAR EXCEPCIONES A NIVEL DE POSTULANTE
    BEGIN
      -------------------------------------------------------------------------------Minimos
    ...
    EXCEPTION
      WHEN TOO_MANY_ROWS THEN 
        INSERT INTO ERRORES_PROC_ASIG_PUNTAJE VALUES (SEQ_ERRORES_PROC_ASIG_PUNTAJE.NEXTVAL,'Procedimiento principal, POSTULANTE: ' || REG_POSTULANTE.RUN_POSTULANTE ,'Existen muchos valores');
      WHEN NO_DATA_FOUND THEN
        INSERT INTO ERRORES_PROC_ASIG_PUNTAJE VALUES (SEQ_ERRORES_PROC_ASIG_PUNTAJE.NEXTVAL,'Procedimiento principal, POSTULANTE: ' || REG_POSTULANTE.RUN_POSTULANTE,'No se encontraron datos');
      WHEN OTHERS THEN
        prc_PostulantePuntaje_Error('Procedimiento principal, POSTULANTE: ' || REG_POSTULANTE.RUN_POSTULANTE, NVL(SQLCODE,0), NVL(sqlerrm,'DESCONOCIDO'));
    END;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      prc_PostulantePuntaje_Error('Procedimiento principal', NVL(SQLCODE,0), NVL(sqlerrm,'DESCONOCIDO'));
